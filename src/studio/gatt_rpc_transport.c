...
/*
 * ZMK Studio's Bluetooth transport, with one fix
 *
 * This is ZMK's app/src/studio/gatt_rpc_transport.c at the commit config/west.yml
 * pins, and it takes that file's place (CONFIG_ZMK_STUDIO_TRANSPORT_BLE=n in
 * minimalpad.conf). The service and its UUIDs are the same, so MinimalPad Studio
 * and the browser app reach it exactly as before.
 *
 * The fix is in notif_rpc_tx_cb(). ZMK's version takes a reference to the host's
 * connection and, when an indication is still waiting for its confirmation,
 * returns without giving the reference back. Every Studio answer longer than one
 * packet goes down that path, so one read of the layout over Bluetooth leaves the
 * connection object held for good. Nothing shows while the link stays up. When
 * the host next disconnects and comes back, Zephyr cannot reuse the held object
 * and gives the new link another one, and every lookup by address
 * (bt_conn_lookup_addr_le) still finds the dead one first. ZMK then believes the
 * active profile is not connected: the endpoint goes to none, no key is sent and
 * Studio gets no answer, while the Mac shows the pad connected. Only a restart of
 * the pad cleared it.
 *
 * Differences from ZMK's file: this comment, the UUIDs inline (ZMK's uuid.h is
 * private to its studio directory), this build's own latency option, the
 * BUILD_ASSERT, and the bt_conn_unref() marked below. Drop this file, and the
 * two options in Kconfig, once ZMK has the fix and config/west.yml pins it.
 */
...
/* Both files would define the same service, and ZMK's would bring the leak back. */
BUILD_ASSERT(!IS_ENABLED(CONFIG_ZMK_STUDIO_TRANSPORT_BLE),
             "Set CONFIG_ZMK_STUDIO_TRANSPORT_BLE=n: this file replaces ZMK's transport");

/* ZMK's src/studio/uuid.h. */
...
#if CONFIG_MINIMALPAD_STUDIO_BLE_PREF_LATENCY < CONFIG_BT_PERIPHERAL_PREF_LATENCY
    struct bt_conn *conn = zmk_ble_active_profile_conn();
    if (conn) {
        uint8_t latency = notif_enabled ? CONFIG_MINIMALPAD_STUDIO_BLE_PREF_LATENCY
...
static void notif_rpc_tx_cb(struct k_work *work) {
    ...
    if (ring_buf_size_get(tx_buf) > 0) {
        ...
        if (ret < 0) {
            // The fix: an indication is still out, and indicate_cb() runs this again when it
            // is confirmed. ZMK returns here still holding the reference taken above.
            bt_conn_unref(conn);
            return;
        }
